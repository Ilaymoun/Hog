...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    x = 0
    while num_rolls > 0:
        avg_roll = make_average(num_rolls, dice)
        if avg_roll >= max_average:
            max_average = avg_roll
            x = num_rolls
        num_rolls -= 1
    return x
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    x = free_bacon(opponent_score) + score
    if is_swap(x, opponent_score) == True:
        if opponent_score > x:
            return 0
        return num_rolls
    if free_bacon(opponent_score) >= margin:
